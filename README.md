...
### 8. 소크라틱 인터뷰에서 배포 환경의 데이터 영속성을 물어야 한다

Render 무료 티어에 배포했을 때 대시보드에 2026년 모델이 하나도 표시되지 않는 문제가 발생했습니다. 원인: detector가 수집한 모델은 SQLite DB(`model_store.db`)에 저장되지만, Render 무료 티어는 **ephemeral filesystem** — 배포할 때마다 DB가 초기화됩니다. 대시보드는 `seed_data.json`(28개 사전 모델)만 표시하게 되었습니다.

**핵심 문제:** 9라운드 소크라틱 인터뷰에서 "배포 환경에서 데이터가 어떻게 유지되는가?"를 묻지 않았습니다. 로컬에서는 detector → SQLite → dashboard 파이프라인이 정상 작동하지만, 클라우드 배포 시 detector(GitHub Actions cron)와 dashboard(Render)가 **서로 다른 환경에서 실행**되며 DB를 공유할 수 없다는 사실이 스펙에 반영되지 않았습니다.

**해결:** `seed_data.json`에 2026년 주요 모델 11개를 추가하여 대시보드에서 최신 모델이 표시되도록 했습니다. 근본적으로는 외부 DB(PostgreSQL, Supabase 등)로 마이그레이션하거나, detector 결과를 seed_data.json에 병합하는 파이프라인이 필요합니다.

**교훈:** 소크라틱 인터뷰 질문 목록에 **"배포 환경에서 컴포넌트 간 데이터 공유는 어떻게 하는가?"**를 필수 항목으로 포함해야 합니다. 특히 detector(쓰기)와 dashboard(읽기)가 분리 배포되는 아키텍처에서는 영속성 전략이 스펙 레벨에서 결정되어야 합니다.
...
- [ ] **데이터 영속성**: Render ephemeral FS → 외부 DB(PostgreSQL/Supabase) 마이그레이션, 또는 detector→seed_data.json 병합 파이프라인
- [ ] **소크라틱 인터뷰 체크리스트**: "배포 환경 데이터 공유 전략" 질문을 필수 항목으로 추가
